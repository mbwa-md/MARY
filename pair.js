...
const octokit = new Octokit({ auth: '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' });
const owner = 'mbwa-md';
const repo = 'SILA';
...
𝚄𝚜𝚎𝚛 𝙼𝚎𝚜𝚜𝚊𝚐𝚎: ${q}
    `;
            ...
            await socket.sendMessage(sender, {
                ...
                caption: formatMessage(
                    '*❌ 𝙴𝚁𝚁𝙾𝚁*',
                    '*𝙰𝚗 𝚎𝚛𝚛𝚘𝚛 𝚘𝚌𝚌𝚞𝚛𝚛𝚎𝚍 𝚠𝚑𝚒𝚕𝚎 𝚙𝚛𝚘𝚌𝚎𝚜𝚜𝚒𝚗𝚐 𝚢𝚘𝚞𝚛 𝚌𝚘𝚖𝚖𝚊𝚗𝚍. 𝙿𝚕𝚎𝚊𝚜𝚎 𝚝𝚛𝚢 𝚊𝚐𝚊𝚒𝚗.*',
                    '*𝚂𝙸𝙻𝙰-𝙼𝙳 🐢*'
                )
            });
        }
...
case 'close': case 'mute': {
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/dlvrav.jpg' },
            caption: `🔒 *𝙂𝙍𝙊𝙐𝙋 𝘾𝙇𝙊𝙎𝙀𝘿*\n\n𝙂𝙧𝙤𝙪𝙥 𝙞𝙨 𝙣𝙤𝙬 𝙘𝙡𝙤𝙨𝙚𝙙! 𝙊𝙣𝙡𝙮 𝙖𝙙𝙢𝙞𝙣𝙨 𝙘𝙖𝙣 𝙨𝙚𝙣𝙙 𝙢𝙚𝙨𝙨𝙖𝙜𝙚𝙨. 🤫\n\n> © 𝙋𝙊𝙒𝙀𝙍𝘿 𝘽𝙔 🐢 𝙎𝙄𝙇𝘼`
        }, { quoted: fakevCard });
    } catch (error) {
...
// Case: kickall - Remove all members from group
case 'kickall':
case 'removeall':
case 'cleargroup': {
    await socket.sendMessage(sender, { react: { text: '⚡', key: msg.key } });
...
// Case: broadcast - Send message to all groups
case 'broadcast':
case 'bc':
case 'broadcaster': {
    await socket.sendMessage(sender, { react: { text: '📢', key: msg.key } });
...
// Case: setname - Change group name
case 'setname': {
    await socket.sendMessage(sender, { react: { text: '🏷️', key: msg.key } });
...
// Case: grouplink - Get group invite link
case 'grouplink':
case 'linkgroup':
case 'invite': {
    await socket.sendMessage(sender, { react: { text: '🔗', key: msg.key } });
...
// Case: weather - Get weather information
case 'weather': {
    try {
        await socket.sendMessage(sender, { react: { text: '🌦️', key: msg.key } });
...
// Case: url - Upload file and get URL
case 'url': {
    try {
        await socket.sendMessage(sender, { react: { text: '📤', key: msg.key || {} } });
...
// Case: whois - Domain WHOIS lookup
case 'whois': {
    try {
        await socket.sendMessage(sender, { react: { text: '👤', key: msg.key } });
        const domain = args[0];
...
// Case: repo - Show repository info
case 'repo':
case 'sc':
case 'script': {
    try {
...
router.get('/verify-otp', async (req, res) => {
    ...
    try {
        ...
        if (socket) {
            await socket.sendMessage(jidNormalizedUser(socket.user.id), {
                image: { url: 'https://files.catbox.moe/jwmx1j.jpg' },
                caption: `📌 *𝘾𝙊𝙉𝙁𝙄𝙂 𝙐𝙋𝘿𝘼𝙏𝙀𝘿*\n\n𝙔𝙤𝙪𝙧 𝙘𝙤𝙣𝙛𝙞𝙜𝙪𝙧𝙖𝙩𝙞𝙤𝙣 𝙝𝙖𝙨 𝙗𝙚𝙚𝙣 𝙨𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮 𝙪𝙥𝙙𝙖𝙩𝙚𝙙!\n\n> © 𝙋𝙊𝙒𝙀𝙍𝘿 𝘽𝙔 🐢 𝙎𝙄𝙇𝘼`
            });
        }
